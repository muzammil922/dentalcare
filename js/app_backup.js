...
class DentalClinicApp {
    ...
    displayPatients(patients, currentPage = 1) {
        ...
        const patientsHTML = `
            <div class="patients-grid-container" style="background: var(--white); border-radius: var(--radius-lg); box-shadow: var(--shadow-md); padding: 1.5rem; margin-bottom: 1rem;">
                <!-- Count Display and Entries Per Page Selector -->
                        ...
                        <div class="patient-row" data-patient-id="${patient.id || 'unknown'}" data-patient-name="${patient.name || 'Unknown Patient'}" style="display: flex; align-items: center; gap: 1.5rem; padding: 1rem; border-bottom: ${index < currentPatients.length - 1 ? '1px solid var(--gray-200)' : 'none'}; transition: background-color 0.2s ease; cursor: pointer;" onmouseover="this.style.backgroundColor='var(--gray-100)'" onmouseout="this.style.backgroundColor='transparent'">
                            ...
                            <div class="patient-status" style="text-align: center; margin-left: auto; display: flex; flex-direction: column; align-items: center; gap: 0.5rem;">
                                ...
                                <label class="status-toggle" style="position: relative; display: inline-block; width: 50px; height: 24px; background: ${isActive ? 'var(--success-color)' : 'var(--error-color)'}; border-radius: 12px; cursor: pointer; transition: all 0.3s ease;">
                                    <input type="checkbox" ${isActive ? 'checked' : ''} style="opacity: 0; width: 0; height: 0;" onchange="window.dentalApp.simpleToggleStatus('${patient.id || 'unknown'}', this.checked, this)">
                            ...
                            <div style="display: flex; gap: 0.5rem; flex-shrink: 0;">
                                <button onclick="window.dentalApp.viewPatientDetails('${patient.id || 'unknown'}')" style="width: 40px; height: 40px; padding: 0; background: var(--primary-light); color: var(--primary-color); border-radius: var(--radius-md); border: none; cursor: pointer; transition: all 0.2s ease-in-out;" title="View Details" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button onclick="window.dentalApp.editPatient('${patient.id || 'unknown'}')" style="width: 40px; height: 40px; padding: 0; background: var(--primary-light); color: var(--primary-color); border-radius: var(--radius-md); border: none; cursor: pointer; transition: all 0.2s ease-in-out;" title="Edit Patient" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick="window.dentalApp.printPatient('${patient.id || 'unknown'}')" style="width: 40px; height: 40px; padding: 0; background: var(--white); color: var(--warning-color); border: 1px solid var(--warning-color); border-radius: var(--radius-md); cursor: pointer; transition: all 0.2s ease-in-out;" title="Print" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
                                    <i class="fas fa-print"></i>
                                </button>
                                <button onclick="window.dentalApp.showDeleteConfirmation('${patient.id || 'unknown'}')" style="width: 40px; height: 40px; padding: 0; background: var(--white); color: var(--error-color); border: 1px solid var(--error-color); border-radius: var(--radius-md); cursor: pointer; transition: all 0.2s ease-in-out;" title="Delete" onmouseover="this.style.transform='scale(1.1)'" onmouseout="this.style.transform='scale(1)'">
    ...
    togglePatientStatus(patientId, isActive) {
        ...
        if (patientIndex !== -1) {
            ...
            // Always reset to page 1 when status changes to avoid pagination issues
            const currentPage = 1;
            
            // Store current filtered patients and display with page 1
            this.currentPatients = filteredPatients;
    ...
    filterPatients(filterType, showToast = true) {
        ...
        if (patientImportBtn) {
            if (filterType === 'all') {
                ...
                patientImportBtn.classList.remove('hidden');
                patientImportBtn.classList.add('show-for-all');
                console.log('Showing patient import button for "all" filter');
            } else {
                // Hide import button for 'active' and 'inactive' filters
                patientImportBtn.classList.add('hidden');
                patientImportBtn.classList.remove('show-for-all');
                console.log('Hiding patient import button for', filterType, 'filter');
            }
        }
...
Status: ${item.status || 'In Stock'}
    ...
    // Improved togglePatientStatus function - Replace the old one with this
    togglePatientStatusImproved(patientId, isActive) {
        console.log('=== IMPROVED TOGGLE FUNCTION CALLED ===');
        console.log('Patient ID:', patientId, 'IsActive:', isActive);
        ...
        try {
            ...
            if (patientIndex !== -1) {
                ...
                console.log('Patient found:', patient.name, 'ID:', patient.id, 'Old status:', oldStatus, 'New status:', newStatus);
                
                // Update the patient status
                patients[patientIndex].status = newStatus;
                this.setStoredData('patients', patients);
                
                // Update the currentPatients array if it exists
                if (this.currentPatients) {
                    const currentPatientIndex = this.currentPatients.findIndex(p => p.id === patientId);
                    if (currentPatientIndex !== -1) {
                        this.currentPatients[currentPatientIndex].status = newStatus;
                    }
                }
                
                this.showToast(`Patient ${patient.name} ${isActive ? 'activated' : 'deactivated'} successfully`, 'success');
                
                // Simple and direct refresh - just redisplay the current patients
                if (this.currentPatients) {
                    this.displayPatients(this.currentPatients, 1); // Always go to page 1 after toggle
                } else {
                    // Fallback: reload all patients
                    const allPatients = this.getStoredData('patients') || [];
                    this.currentPatients = allPatients;
                    this.displayPatients(allPatients, 1);
                }
                
            } else {
                console.log('Patient not found with ID:', patientId);
                this.showToast(`Patient with ID ${patientId} not found`, 'error');
            }
        } catch (error) {
            console.error('Error in togglePatientStatusImproved:', error);
    ...
    // Simple test function to verify toggle functionality
    testToggleFunction(patientId) {
        console.log('=== TESTING TOGGLE FUNCTION ===');
        const patients = this.getStoredData('patients') || [];
        const patient = patients.find(p => p.id === patientId);
        
        if (patient) {
            console.log('Testing toggle for patient:', patient.name, 'ID:', patient.id, 'Current status:', patient.status);
            const newStatus = patient.status === 'active' ? false : true;
            this.togglePatientStatusImproved(patientId, newStatus);
        } else {
            console.log('Patient not found with ID:', patientId);
        }
    }

    // Simple and direct toggle function that updates the UI immediately
    simpleToggleStatus(patientId, isActive, toggleElement) {
        console.log('=== SIMPLE TOGGLE CALLED ===');
        console.log('Patient ID:', patientId, 'New Status:', isActive);
        
        try {
            // Get the patient row element
            const patientRow = toggleElement.closest('.patient-row');
            if (!patientRow) {
                console.error('Patient row not found');
                return;
            }
            
            // Get the patient name from the DOM - THIS IS THE SOURCE OF TRUTH
            const patientNameElement = patientRow.querySelector('.patient-name');
            const domPatientName = patientNameElement ? patientNameElement.textContent.trim() : 'Unknown';
            console.log('Patient name from DOM (SOURCE OF TRUTH):', domPatientName);
            ...
            if (patient) {
                patientIndex = allPatients.findIndex(p => p.id === patientId);
                console.log('Patient found by ID:', patient.name, 'ID:', patient.id);
            }
            
            // SECOND: If ID match fails or name doesn't match DOM, find by name
            if (!patient || patient.name !== domPatientName) {
                console.log('ID match failed or name mismatch, searching by name...');
                patient = allPatients.find(p => p.name && p.name.toLowerCase() === domPatientName.toLowerCase());
                
                if (patient) {
                    patientIndex = allPatients.findIndex(p => p.name && p.name.toLowerCase() === domPatientName.toLowerCase());
                    console.log('Patient found by name:', patient.name, 'ID:', patient.id);
                }
            }
            
            // THIRD: If still no match, show error
            if (!patient) {
                console.error('Patient not found in storage by ID or name');
                console.log('DOM Patient Name:', domPatientName);
                console.log('Available patients:', allPatients.map(p => ({ id: p.id, name: p.name })));
                this.showToast(`Patient "${domPatientName}" not found in storage`, 'error');
                return;
            }
            ...
            console.log('Final patient data:', { 
                ...
                oldStatus, 
                newStatus,
                domName: domPatientName,
                matchMethod: patient.id === patientId ? 'ID match' : 'Name match'
            });
            
            // Update the main storage
            allPatients[patientIndex].status = newStatus;
            this.setStoredData('patients', allPatients);
            
            // Also update currentPatients array if it exists (for UI consistency)
            if (this.currentPatients) {
                const currentIndex = this.currentPatients.findIndex(p => p.id === patient.id);
                if (currentIndex !== -1) {
                    this.currentPatients[currentIndex].status = newStatus;
                    console.log('Updated currentPatients array for UI consistency');
                } else {
                    console.warn('Patient not found in currentPatients array - this is normal for pagination');
                }
            }
            
            // Update the toggle element's parent label background
            const toggleLabel = toggleElement.closest('.status-toggle');
            if (toggleLabel) {
                toggleLabel.style.background = isActive ? 'var(--success-color)' : 'var(--error-color)';
            }
            
            // Update the toggle slider position
            const toggleSlider = toggleLabel.querySelector('.toggle-slider');
            if (toggleSlider) {
                toggleSlider.style.left = isActive ? '26px' : '3px';
            }
            
            // Update the status text
            const statusText = patientRow.querySelector('.patient-status div:last-child');
            if (statusText) {
                statusText.textContent = isActive ? 'Active' : 'Inactive';
                statusText.style.color = isActive ? 'var(--success-color)' : 'var(--error-color)';
            }
            
            // Show success message with the DOM patient name (what user sees)
            this.showToast(`Patient ${domPatientName} ${isActive ? 'activated' : 'deactivated'} successfully`, 'success');
            
            console.log('Toggle completed successfully for patient:', domPatientName);
        
        } catch (error) {
            console.error('Error in simpleToggleStatus:', error);
            this.showToast('Error updating patient status', 'error');
    ...
    // Test toggle on specific page to debug pagination issues
    testToggleOnPage(pageNumber) {
        console.log(`=== TESTING TOGGLE ON PAGE ${pageNumber} ===`);
        ...
        console.log(`Page ${pageNumber}: Patients ${startIndex + 1}-${endIndex}`);
        console.log('Patients on this page:', pagePatients.map(p => ({ id: p.id, name: p.name, status: p.status })));
        
        // Test first patient on this page
        if (pagePatients.length > 0) {
            const testPatient = pagePatients[0];
            console.log(`Testing toggle for first patient on page ${pageNumber}:`, testPatient.name);
            
            // Find the toggle element in the DOM
            const patientRow = document.querySelector(`[data-patient-id="${testPatient.id}"]`);
            if (patientRow) {
                const toggleInput = patientRow.querySelector('input[type="checkbox"]');
                if (toggleInput) {
                    console.log('Found toggle element, testing...');
                    const newStatus = testPatient.status === 'active' ? false : true;
                    this.simpleToggleStatus(testPatient.id, newStatus, toggleInput);
                } else {
                    console.error('Toggle input not found');
                }
            } else {
                console.error('Patient row not found in DOM - page may not be displayed');
            }
        }
        
        return { pageNumber, startIndex, endIndex, patients: pagePatients };
    }

    // Test toggle for a specific patient by name
    testToggleByName(patientName) {
        console.log(`=== TESTING TOGGLE FOR PATIENT: ${patientName} ===`);
        
        const patients = this.getStoredData('patients') || [];
        const patient = patients.find(p => p.name && p.name.toLowerCase().includes(patientName.toLowerCase()));
        ...
        if (patient) {
            ...
            // Test the toggle
            const newStatus = patient.status === 'active' ? false : true;
            console.log('Testing toggle to:', newStatus);
            
            // Find the toggle element in the DOM
            const patientRow = document.querySelector(`[data-patient-name*="${patient.name}"]`);
            if (patientRow) {
                const toggleInput = patientRow.querySelector('input[type="checkbox"]');
                if (toggleInput) {
                    console.log('Found toggle element, testing...');
                    this.simpleToggleStatus(patient.id, newStatus, toggleInput);
                } else {
                    console.error('Toggle input not found');
                }
            } else {
                console.error('Patient row not found in DOM');
            }
        } else {
            console.log(`Patient with name containing "${patientName}" not found`);
            console.log('Available patients:', patients.map(p => p.name));
        }
    }
