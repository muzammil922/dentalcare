...
class DentalClinicApp {
    constructor() {
        ...
        this.billingPerPage = 10; // Default invoices per page
    ...
    displayBilling(invoices, currentPage = 1) {
        ...
        const billingHTML = `
            <div class="billing-grid-container" style="background: var(--white); border-radius: var(--radius-lg); box-shadow: var(--shadow-md); padding: 1.5rem; margin-bottom: 1rem;">
                ...
                <div style="display: flex; align-items: center; gap: 1.5rem; padding: 1rem; background: var(--gray-50); border-bottom: 1px solid var(--gray-200); font-weight: 600; color: var(--gray-700);">
                    <div style="min-width: 120px; display: flex; align-items: center; gap: 1rem;">
                        <input type="checkbox" id="select-all-patients" onchange="window.dentalApp.toggleSelectAllInvoice(this.checked)" style="width: 14px; height: 14px; cursor: pointer;">
                        <span style="font-size: 0.875rem; color: var(--primary-color);">Select All</span>
                    </div>
...
Status: ${item.status || 'In Stock'}
    ...
    loadEntriesPerPagePreference() {
        ...
        // Also load appointments preference
        const savedAppointments = localStorage.getItem('dentalApp_appointmentsPerPage');
        if (savedAppointments) {
            this.appointmentsPerPage = savedAppointments === 'all' ? 'all' : (parseInt(savedAppointments) || 10);
        } else {
            this.appointmentsPerPage = 10;
        }
        console.log('Loaded appointments entries per page:', this.appointmentsPerPage);
    }

    // Change appointments entries per page and refresh
    changeAppointmentsEntriesPerPage(newEntriesPerPage) {
        if (newEntriesPerPage === 'all') {
            this.appointmentsPerPage = 'all';
        } else {
            this.appointmentsPerPage = parseInt(newEntriesPerPage) || 10;
        }
        localStorage.setItem('dentalApp_appointmentsPerPage', this.appointmentsPerPage);
        const pageAttr = document.getElementById('appointments-list')?.getAttribute('data-current-page');
        const currentPage = pageAttr ? parseInt(pageAttr) || 1 : 1;
        this.displayAppointments(this.currentAppointments || (this.getStoredData('appointments') || []), 1);
        this.showToast(`Now showing ${this.appointmentsPerPage === 'all' ? 'all' : this.appointmentsPerPage} appointments per page`, 'success');
    }
